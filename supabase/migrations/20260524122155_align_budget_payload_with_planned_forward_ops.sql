-- Aligne get_budget_page_payload avec les sources utilisées par v_optimization_monthly_budget_forecast.
-- Avant : seule la table `budgets` alimentait le budget du mois sélectionné.
-- Après  : les opérations planifiées budget_impact='additional_commitment' du mois sélectionné
--          sont aussi injectées dans selected_budget_by_category, exactement comme la CTE `forward`
--          de v_optimization_monthly_budget_forecast le fait pour la page épargne.
-- Impact concret : la crèche (Alexis, oct 2026–juil 2027, 295 €/mois) apparaît désormais
--                  dans les graphiques et les enveloppes de l'onglet Budgets pour les mois futurs.

CREATE OR REPLACE FUNCTION budget_dashboard.get_budget_page_payload(p_user_id uuid, p_period_year integer, p_period_month integer, p_months_back integer DEFAULT 6)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
begin
  ...
  actual_monthly_category_raw as (
    select
      ...
      vmca.period_month,
      vmca.category_id,
      sum(vmca.actual_amount)::numeric as actual_amount
    from budget_dashboard.v_monthly_category_actuals_clean vmca
    join window_periods wp
      on wp.period_year = vmca.period_year
     and wp.period_month = vmca.period_month
    join mapped_categories mc
      on mc.category_id = vmca.category_id
  ...
  -- Opérations planifiées additional_commitment pour le mois sélectionné uniquement.
  -- Miroir de la CTE `forward` dans v_optimization_monthly_budget_forecast.
  planned_forward_for_selected_period as (
    select
      po.category_id,
      sum(round(po.planned_amount * po.personal_share_ratio, 2))::numeric as planned_amount
    ...
    cross join lateral (
      ...
      from generate_series(
        ...
        case
          when po.is_recurring then coalesce(po.recurrence_end_date, po.planned_date)
          else po.planned_date
        end::timestamptz,
        interval '1 month'
      ) gs
    ) occ
    join mapped_categories mc on mc.category_id = po.category_id
    where po.user_id = p_user_id
      and po.budget_impact = 'additional_commitment'
      and po.status = 'planned'
      and po.flow_type = 'expense'
      and date_trunc('month', occ.occ_date)::date = v_selected_month_start
  ...
  actual_available_months as (
    ...
    where exists (
      ...
      from budget_dashboard.v_monthly_category_actuals_clean vmca
      where vmca.user_id = p_user_id
        and vmca.period_year = wp.period_year
        and vmca.period_month = wp.period_month
    )
  ),
  ...
  -- Budget du mois sélectionné : lignes budgétaires + opérations planifiées additional_commitment.
  -- Remarque : les mois historiques (window) n'intègrent PAS les planned ops — seul le mois courant.
  selected_budget_by_category as (
    ...
    from (
      ...
      select
        pfsp.category_id,
        pfsp.planned_amount as budget_amount
      from planned_forward_for_selected_period pfsp
    ) combined
  ...
  category_actual_6m_sum as (
    ...
    from mapped_categories mc
    left join actual_available_months aam on true
  ...
  category_budget_6m_sum as (
    ...
    from mapped_categories mc
    left join budget_available_months bam on true
  ...
  by_bucket_selected as (
    ...
    from bucket_list bl
    left join selected_category_rows scr on scr.budget_bucket = bl.budget_bucket
  ...
  bucket_actual_6m_sum as (
    ...
    left join (
      ...
      from actual_monthly_category_raw amcr
      join mapped_categories mc on mc.category_id = amcr.category_id
  ...
  bucket_budget_6m_sum as (
    ...
    left join (
      ...
      from budget_monthly_category_raw bmcr
      join mapped_categories mc on mc.category_id = bmcr.category_id
  ...
  history_months as (
    ...
    union
    select aam.period_year, aam.period_month from actual_available_months aam
    union
    select bam.period_year, bam.period_month from budget_available_months bam
  ),
  ...
  history_rows as (
    select
      ...
      case when coalesce(bmtr.budget_total, 0) = 0 then null
  ...
  select jsonb_build_object(
    ...
    'by_parent_category', coalesce(
      (select jsonb_agg(jsonb_build_object(
        ...
        from by_parent_rows bpr cross join selected_totals st),
      '[]'::jsonb
    ),
