-- ============================================================
-- Fix voyage projection: replace category-budget-based formula
-- with trips.planned_budget as single source of truth
--
-- Root cause:
--   v_annual_projection_overview_2026 and
--   v_category_annual_cost_projection_2026 both used the
--   `budgets` table (category lines: Logement voyage,
--   Repas voyage, Trajet voyage, etc.) to project voyage
--   spending → 4832€ projected.
--
--   The trips system (trips.planned_budget) is the canonical
--   source of truth for voyage budgets → 2765€ projected.
--   Both mechanisms coexisted and diverged.
--
-- Fix:
--   For voyage bucket, replace budget_remaining with
--   sum(trips.planned_budget) for trips starting after
--   completed_months. For v_category_annual_cost_projection_2026,
--   the trip remaining is distributed proportionally across
--   voyage categories by their budget_annual_amount share so
--   that both views sum to the same total.
--
-- Result: voyage projected_annual 4832€ → 2765€,
--   consistent across both projection surfaces.
-- ============================================================


-- ============================================================
-- 1/2  v_annual_projection_overview_2026
-- ============================================================
...
budget_remaining AS (
    -- 'voyage' intentionally excluded: replaced by voyage_trip_remaining below
    SELECT
        cbm.budget_bucket,
    ...
    WHERE bp.period_year   = p.period_year
      ...
      AND cbm.budget_bucket = ANY(ARRAY[
            'revenu', 'socle_fixe', 'variable_essentielle',
            'discretionnaire', 'provision', 'epargne'])
    GROUP BY cbm.budget_bucket
),
voyage_trip_remaining AS (
    SELECT COALESCE(sum(t.planned_budget), 0)::numeric(14,2) AS amount
    FROM budget_dashboard.trips t
    CROSS JOIN params p
    WHERE t.year = p.period_year
      AND EXTRACT(month FROM t.start_date)::int > p.completed_months
      AND t.planned_budget IS NOT NULL
),
bucket_projection AS (
    SELECT
        ...
        CASE
            WHEN bs.budget_bucket = 'voyage' THEN vtr.amount
            ELSE COALESCE(br.budget_remaining_amount, 0)
        END::numeric(14,2)                                     AS projected_remaining_amount,
        (
            ...
            CASE
                ...
                ELSE COALESCE(br.budget_remaining_amount, 0)
            END
        )::numeric(14,2)                                       AS projected_annual_amount
    FROM bucket_scope bs
    CROSS JOIN params p
    CROSS JOIN voyage_trip_remaining vtr
...
-- ============================================================
-- 2/2  v_category_annual_cost_projection_2026
-- ============================================================
...
voyage_trip_remaining AS (
    ...
    WHERE t.year = p_1.period_year
      AND EXTRACT(month FROM t.start_date)::int > p_1.completed_months
      AND t.planned_budget IS NOT NULL
),
voyage_budget_annual_total AS (
    SELECT COALESCE(sum(b_1.budget_annual_amount), 0)::numeric(14,2) AS total
    FROM budgets b_1
...
SELECT
    ...
    END AS avg_monthly_ytd_amount,
    CASE
        WHEN cs.budget_bucket = 'voyage' THEN
            CASE
                WHEN vbat.total > 0
                THEN round(COALESCE(b.budget_annual_amount, 0) / vbat.total * vtr.amount, 2)
                ELSE 0
            END
        ELSE
            (COALESCE(b.budget_annual_amount, 0) - COALESCE(b.budget_ytd_amount, 0))
    END::numeric(14,2) AS projected_remaining_amount,
    CASE
        WHEN cs.budget_bucket = 'voyage' THEN
            COALESCE(a.actual_ytd_amount, 0) +
            ...
            END
        ELSE
            (COALESCE(a.actual_ytd_amount, 0) + COALESCE(b.budget_annual_amount, 0) - COALESCE(b.budget_ytd_amount, 0))
    END::numeric(14,2) AS projected_annual_amount,
...
CROSS JOIN params p
CROSS JOIN voyage_trip_remaining vtr
CROSS JOIN voyage_budget_annual_total vbat
