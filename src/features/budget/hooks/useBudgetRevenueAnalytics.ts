...
async function fetchBudgetRevenueAnalytics(
  userId: string,
...
export function useBudgetRevenueAnalytics(periodYear?: number, periodMonth?: number) {
  const { user, loading: authLoading } = useAuth()
  const userId = user?.id ?? null

  const query = useQuery({
    queryKey: ['budget-revenue-analytics', userId, periodYear, periodMonth],
    queryFn: () => fetchBudgetRevenueAnalytics(userId!, periodYear, periodMonth),
